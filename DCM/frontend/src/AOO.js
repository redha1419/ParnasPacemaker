...
class AOO extends React.Component {
    ...
    //this function is called when AOO mode becomes active - sends a request to the backend to save programmable parameters for current user
    componentDidMount(){
    ...
    submit(e){
      ...
      if(!error){
        ...
        axios.post('http://localhost:3000/pace',  {
          username: this.context.username,
          mode: mode,
          config: {lower, upper, atrial_amp, atrial_pw, maximum_sensor_rate,reaction_time,recovery_time,response_factor,activity_threshold}
          })
          .then( res =>{
